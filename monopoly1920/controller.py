...
class Controller(observer.Observer):
    ...
    def _roll_dice(self):
        ...
        if dice1 == dice2:
            ...
            if self.__roll_count >= 3:
                ...
                self.__roll_count = 0
                return dice1, dice2  # prevent further actions
    ...
    def handle_jail_turn(self, player):
        ...
        if player.get_jail_turns() >= 3:
            player.leave_jail()
            player._money -= 50  # Assume player has enough money
            observer.Event("update_state", f"{player.name} paid $50 after 3 turns and leaves jail.")
            return

        roll = self._roll_dice()
        if roll[0] == roll[1]:
            player.leave_jail()
            observer.Event("update_state", f"{player.name} rolled doubles and escapes jail!")
        else:
            player.increment_jail_turn()
            observer.Event("update_state", f"{player.name} did not roll doubles and remains in jail (turn {player.get_jail_turns()}).")